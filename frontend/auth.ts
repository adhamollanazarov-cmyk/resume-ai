  ...
  callbacks: {
    ...
    async jwt({ token, user, trigger }) {
      ...
      try {
        ...
        token.userId = String(syncedUser.id);
        ...
        logAuthIssue("User sync failed during JWT refresh, preserving the OAuth session.", error, {
          provider: "github",
        });

        if (!token.plan || (token.plan !== "free" && token.plan !== "pro")) {
          token.plan = "free";
        }

        if (typeof token.analysisCount !== "number") {
